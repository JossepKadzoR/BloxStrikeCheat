...
local function createToggle(name, parent, callback)
    ...
    Btn.Size = UDim2.new(0, 45, 0, 24); Btn.Position = UDim2.new(1, -55, 0.5, -12); Btn.BackgroundColor3 = Color3.fromRGB(45, 45, 50); Btn.Text = ""; Btn.Parent = Frame
    Instance.new("UICorner", Btn).CornerRadius = UDim.new(1, 0)

    local Circle = Instance.new("Frame")
    Circle.Size = UDim2.new(0, 18, 0, 18); Circle.Position = UDim2.new(0, 3, 0.5, -9); Circle.BackgroundColor3 = Color3.fromRGB(255, 255, 255); Circle.Parent = Btn
...
local function createSlider(name, parent, min, max, default, callback)
    local Frame = Instance.new("Frame"); Frame.Size = UDim2.new(1, -10, 0, 60); Frame.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Frame.Parent = parent
    Instance.new("UICorner", Frame)
    local Label = Instance.new("TextLabel"); Label.Text = "  " .. name .. ": " .. default; Label.Size = UDim2.new(1, 0, 0, 30); Label.TextColor3 = Color3.fromRGB(200, 200, 200); Label.BackgroundTransparency = 1; Label.TextXAlignment = "Left"; Label.Parent = Frame
    local SliderBack = Instance.new("Frame"); SliderBack.Size = UDim2.new(0.9, 0, 0, 6); SliderBack.Position = UDim2.new(0.05, 0, 0.7, 0); SliderBack.BackgroundColor3 = Color3.fromRGB(45, 45, 50); SliderBack.Parent = Frame
...
createSlider("Moc Szybkości", MoveTab, 16, 250, 50, function(v) walkSpeedValue = v end)
createToggle("FlyJump (Light)", MoveTab, function(s) flyJumpActive = s end)

-- SILNIK SKRYPTU (Pętla wymuszająca)
RunService.PostSimulation:Connect(function()
    local char = Player.Character
    local hum = char and char:FindFirstChildWhichIsA("Humanoid")
    
    if hum then
        -- Naprawa SpeedHacka (Metoda Direct)
        if speedActive or flyJumpActive then
            hum.WalkSpeed = flyJumpActive and 80 or walkSpeedValue
        else
            -- Jeśli wyłączone, przywracamy normę tylko jeśli skrypt ją zmienił
        ...
        if flyJumpActive then
            hum.JumpPower = 70 -- Lekko większy skok (domyślnie jest 50)
